...
pub fn show(ui: &mut egui::Ui, settings: &mut Settings) {
    egui::CollapsingHeader::new("Display")
        ...
        .show(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                ui.label("VSync")
                    .on_hover_text("Controls how frames are presented to the display.");
    ...
    egui::CollapsingHeader::new("Post Processing").default_open(true).show(ui, |ui| {
        ...
        ui.checkbox(&mut settings.color_compression, "Color compression").on_hover_text(
            "Posterizes the final image with ordered dithering for a harsher retro/compressed look.",
        );
        if settings.color_compression {
            ui.horizontal(|ui| {
                ui.label("Color steps")
                    .on_hover_text("Lower values crush more colors and make the effect harsher.");
                ui.add(
                    egui::Slider::new(&mut settings.color_compression_steps, 2.0..=64.0)
                        .fixed_decimals(0),
                );
            });
            ...
            ui.horizontal(|ui| {
                ui.label("Dither strength")
                    .on_hover_text("How much ordered dithering breaks up flat posterization bands.");
                ui.add(
                    egui::Slider::new(&mut settings.color_compression_dither, 0.0..=1.5)
                        .fixed_decimals(2),
                );
            });
        }

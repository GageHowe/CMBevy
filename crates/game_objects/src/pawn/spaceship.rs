...
/// radians/sec from keyboard Q/E roll
...
impl GameObject for SpaceshipPawnComponent {
    fn spawn(entity: Entity, cmd: &net::message::SpawnCommand, world: &mut World) {
        ...
        world.entity_mut(entity).insert((SpaceshipPawnComponent, Transform::from(transform), cmd.net_id.clone()));
        let rb_handle = {
...
/// Gathers keyboard + mouse into a SpaceshipInput each FixedPreUpdate.
/// Mouse delta drives yaw/pitch; Q/E drive roll.
fn gather_spaceship_input(
    ...
    let mut input = SpaceshipInput::default();
    ...
    // mouse drives yaw/pitch as per-tick angle deltas
    let s = sensitivity.0;
...
pub fn apply_spaceship_movement(
    ...
    // direct angular velocity control: mouse gives per-tick angle deltas; keyboard gives ±1 roll
    // multiply mouse deltas by 60 to convert tick-rate angle → rad/s for Rapier's angvel

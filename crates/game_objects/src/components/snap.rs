...
use bevy::prelude::*;
use physics::physics_world::{PhysicsWorld, RigidBodyHandleComponent, rb_pos, rb_rot, step_physics};
...
fn snap_eval(center: Vec3, axis: Option<Vec3>, inner_r: f32, outer_r: f32, half_width: f32, pos: Vec3) -> Option<(Vec3, f32)> {
    match axis {
        None => in_spherical_zone(center, inner_r, outer_r, pos),
        Some(ax) => in_ring_zone(center, ax, inner_r, outer_r, half_width, pos).map(|(d, dist)| (-d, dist)),
    }
}
...
fn orient_body_to_up(rb: &mut RigidBody, current_rot: Quat, desired_up: Vec3, dt: f32) {
    ...
    rb.set_rotation(current_rot.slerp(target, (ORIENT_SPEED * dt).min(1.0)), false);
}
...
pub fn orient_bipeds_to_snap_sources(
    ...
    let entries: Vec<(Entity, Vec3, Option<Vec3>, f32, f32, f32)> = sources
        ...
        .filter_map(|(entity, src, handle)| {
            ...
            Some((entity, rb_pos(rb), axis, src.inner_radius as f32, src.radius as f32, src.half_width as f32))
        })
        .collect();

    for (handle, mut biped) in bipeds.iter_mut() {
        let (pos, rot) = {
            let Some(rb) = world.rigid_body_set.get(handle.0) else { continue };
            (rb_pos(rb), rb_rot(rb))
        };
        let best = entries
            .iter()
            .filter_map(|&(e, c, ax, ir, or_, hw)| snap_eval(c, ax, ir, or_, hw, pos).map(|(up, d)| (e, up, d)))
            .min_by(|a, b| a.2.partial_cmp(&b.2).unwrap_or(Ordering::Equal));
        let Some(rb) = world.rigid_body_set.get_mut(handle.0) else { continue };
        match best {
...
pub fn orient_bipeds_to_snap_sources_impulses(
    ...
    let entries: Vec<(Vec3, Option<Vec3>, f32, f32, f32)> = sources
        ...
        .filter_map(|(src, handle)| {
            ...
            Some((rb_pos(rb), axis, src.inner_radius as f32, src.radius as f32, src.half_width as f32))
        })
        .collect();
    ...
    for (src, gt) in sources.iter() {
        ...
        match src.kind {
            ...
            SnapKind::Axis(dir) => {
                ...
                super::draw_zone_circles(&mut gizmos, pos, normal, src.inner_radius as f32, src.radius as f32, src.half_width as f32, Color::srgba(0.4, 0.8, 1.0, 0.6));
            }
        }
    }
}

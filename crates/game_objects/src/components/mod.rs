...
pub fn draw_zone_circles(
    ...
    let offsets: &[f32] = if half_width > 0.0 { &[-half_width, half_width] } else { &[0.0] };
    for &off in offsets {
        let iso = Isometry3d::new(pos + normal * off, rot);
        if inner_r > 0.0 { gizmos.circle(iso, inner_r, color); }
        if outer_r > 0.0 { gizmos.circle(iso, outer_r, color); }
    }
}
...
pub fn in_spherical_zone(
    center: Vec3, inner_r: f32, outer_r: f32, pos: Vec3,
) -> Option<(Vec3, f32)> {
    let d = pos - center;
    let dist = d.length();
...
pub fn in_ring_zone(
    center: Vec3, axis: Vec3, inner_r: f32, outer_r: f32, half_width: f32, pos: Vec3,
) -> Option<(Vec3, f32)> {
    let axial = (pos - center).dot(axis);
    if half_width > 0.0 && axial.abs() > half_width { return None; }

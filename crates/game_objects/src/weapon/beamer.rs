...
impl Weapon for BeamerComponent {
    const MODEL_PATH: &'static str = "models/placeholder_beamer.glb#Scene0";
    ...
    const ZOOM_MULTIPLIER: f32 = 1.8;
    const MAGAZINE_SIZE: u16 = MAGAZINE_SIZE;
    const RESERVE_AMMO: u16 = RESERVE_AMMO;
    const RELOAD_TICKS: u16 = RELOAD_TICKS;
    const FIRE_COOLDOWN_TICKS: u16 = COOLDOWN_TICKS;
    // Unused for beamer. Keep a valid kind to satisfy the existing Weapon trait.
    const PROJECTILE_KIND: net::message::GameObjectKind =
        net::message::GameObjectKind::PistolProjectile;
    const FIRE_PROJECTILE: super::FireProjectileFn = unused_fire_projectile;
    ...
    fn fixed_update(
        ...
        if self.phase == BeamPhase::Idle {
            if !super::can_fire(ctx.weapon_state) {
                return;
            }
            self.phase = BeamPhase::Charging;
            self.phase_started_tick = ctx.tick;
            self.damage_tick_accum = 0;
            self.beam_origin = ctx.origin;
            self.beam_dir = ctx.aim_dir.normalize_or_zero();
            #[cfg(feature = "client")]
        ...
        if self.phase == BeamPhase::Charging {
            self.beam_origin = ctx.origin;
            self.beam_dir = ctx.aim_dir.normalize_or_zero();
            if ctx.tick.saturating_sub(self.phase_started_tick) + 1 < CHARGE_TICKS as u64 {
                return;
            }
            self.phase = BeamPhase::Beaming;
            self.phase_started_tick = ctx.tick;
            self.damage_tick_accum = DAMAGE_INTERVAL_TICKS - 1;
            #[cfg(feature = "client")]
            if let (Some(quic), Some(weapon_net_id)) = (ctx.quic.as_deref_mut(), ctx.net_id) {
                quic.send_to_server(
                    Channel::Ordered,
        ...
        if self.phase != BeamPhase::Beaming {
            return;
        }

        self.beam_origin = ctx.origin;
        self.beam_dir = ctx.aim_dir.normalize_or_zero();
        if self.beam_dir == Vec3::ZERO {
            return;
        }

        self.damage_tick_accum += 1;
        if self.damage_tick_accum < DAMAGE_INTERVAL_TICKS {
            return;
        }
        self.damage_tick_accum = 0;
        ...
        #[cfg(feature = "client")]
        let hit = beam_hit(_world, ctx.origin, self.beam_dir, ctx.shooter);
        #[cfg(feature = "client")]
        if let Some(weapon_net_id) = ctx.net_id.cloned() {
            queue_beam_hit_report(_commands, weapon_net_id, ctx.origin, self.beam_dir, hit);
...
) -> Option<crate::projectile::FiredProjectile> {
    None
}

...
impl Weapon for LobberComponent {
    ...
    const PREDICTION_PROJECTILE_SPEED: Option<f32> = Some(lobber::SPEED);
    ...
    const PROJECTILE_KIND: net::message::GameObjectKind =
        net::message::GameObjectKind::LobberProjectile;
    const FIRE_PROJECTILE: super::FireProjectileFn =
        <lobber::LobberProjectile as crate::projectile::Projectile>::fire_authoritative;
    ...
    fn fixed_update(
        ...
        helpers::fire_projectile(ctx, world, commands, lobber::SPEED, lobber::spawn);
        ...
        helpers::apply_local_predicted_impulse(
            ctx,
            world,
            -ctx.aim_dir * lobber::shooter_knockback(helpers::shooter_mass(world, ctx.shooter)),
        );

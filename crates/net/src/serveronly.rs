...
use crate::quic::{Channel, ConnectionId, QuicManager, SendTarget, TransportEvent, drain_transport_events, ensure_rustls_crypto_provider, send_on_connection, spawn_connection_tasks};
...
pub(crate) struct ServerTransport { pub(crate) tx: mpsc::UnboundedSender<ServerCommand>, pub(crate) rx: Mutex<std::sync::mpsc::Receiver<TransportEvent>> }
struct ServerConnection { connection: quinn::Connection, ordered_tx: mpsc::UnboundedSender<MsgType> }

impl Plugin for NetServerPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<QuicManager>().add_systems(PreUpdate, process_inbound_server).add_systems(PostUpdate, flush_outbound_server);
    }
}
...
pub fn process_inbound_server(mut quic: ResMut<QuicManager>) {
    let Some(transport) = &quic.server_transport else { return };
...
fn run_server_worker(
    ...
    let runtime = match Builder::new_multi_thread().enable_all().build() { Ok(runtime) => runtime, Err(e) => { eprintln!("Failed to build server runtime: {e}"); return; } };
    runtime.block_on(async move {
        let endpoint = match make_server_endpoint(addr) { Ok(endpoint) => endpoint, Err(e) => { eprintln!("Failed to start QUIC server: {e}"); return; } };
        ...
        while let Some(command) = cmd_rx.recv().await {
            match command {
                ServerCommand::Accepted(connection) => {
                    ...
                    let ordered_tx = spawn_connection_tasks(conn_id, connection.clone(), event_tx.clone(), move |conn_id| {
                ...
                ServerCommand::Send { target, channel, msg } => send_to_targets(&connections, target, channel, &msg).await,
...
async fn send_to_targets(
    ...
    match target {
        SendTarget::All => {
            for connection in connections.values() { send_on_connection(&connection.connection, &connection.ordered_tx, channel, msg).await; }
        }
        SendTarget::One(conn_id) => {
            if let Some(connection) = connections.get(&conn_id) { send_on_connection(&connection.connection, &connection.ordered_tx, channel, msg).await; }
        }
        SendTarget::AllExcept(excluded) => {
            for (&conn_id, connection) in connections.iter() {
                if conn_id != excluded { send_on_connection(&connection.connection, &connection.ordered_tx, channel, msg).await; }
            }
        }
    }
}
...
fn make_server_endpoint(addr: SocketAddr) -> Result<quinn::Endpoint, String> {
    ...
    let cert = rcgen::generate_simple_self_signed(vec!["localhost".into(), addr.ip().to_string()]).map_err(|e| format!("generate cert: {e}"))?;
    let key = rustls::pki_types::PrivatePkcs8KeyDer::from(cert.key_pair.serialize_der());
    let cert_der = cert.cert.der().clone();
    let server_crypto = rustls::ServerConfig::builder().with_no_client_auth().with_single_cert(vec![cert_der], key.into()).map_err(|e| format!("server tls: {e}"))?;
    let mut server_config = quinn::ServerConfig::with_crypto(Arc::new(QuicServerConfig::try_from(server_crypto).map_err(|e| format!("server quic tls: {e}"))?));

...
fn cleanup_world_gc_entities(
    ...
    for (entity, _net_id, body, _spawner_gc, _gc) in &mut gc_q {
    ...
    for (entity, net_id, body, spawner_gc, mut gc) in &mut gc_q {
        let Some(rb) = physics.rigid_body_set.get(body.0) else {
            continue;
        };
        ...
        eprintln!(
            "gc despawned entity={entity} net_id={:?}",
            net_id.map(|id| id.0)
        );
    ...
    for candidate in cull_candidates {
        ...
        if let Ok((_, net_id, _, spawner_gc, _)) = gc_q.get_mut(candidate.entity) {
            state.notify_spawner_despawn(spawner_gc, candidate.entity);
            eprintln!(
                "gc hard-cap despawned entity={} net_id={:?}",
                candidate.entity,
                net_id.map(|id| id.0)
            );
        }
